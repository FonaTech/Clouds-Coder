...
<p align="center">
  ...
  <a href="./log/CHANGELOG-2026-05-02.md">2026-05-02 Changelog (EN/中文/日本語)</a><br />
...
Clouds Coder には **TF-Graph_IDF** と呼ぶ検索エンジンが内蔵されています。**BM25 語彙スコアリング**・知識グラフ位相・自動コミュニティ検出・シンボル完全一致ブースト・マルチルートクエリオーケストレーションを組み合わせ、語彙のみ（埋め込みモデル不要）でありながら散文とソースコードの双方で高い召回を狙ってチューニングされています。
...
flowchart TB
  ...
  subgraph QUERY["クエリレイヤー"]
    ...
    F["FAST パス<br/>ベクトル検索<br/>語彙 × 0.80 + graph_bonus + symbol_boost"]
...
検索された各チャンクは **BM25 語彙コンポーネント**、**グラフボーナス**、**シンボル完全一致ブースト** で構成されるスコアを受け取ります。検索は語彙のみで、いかなる段階でも埋め込みモデルは関与しません：

```
final_score = 語彙 × 0.80 + graph_bonus + symbol_boost   （Data RAG・Code RAG 共通）

bm25        = Σ_t  q_weight_t × (tf_t × (k1+1)) / (tf_t + k1 × (1 − b + b × dl/avgdl))
              k1 = RAG_BM25_K1 = 1.5、b = RAG_BM25_B = 0.6
              dl = チャンク長、avgdl = コーパス平均チャンク長

語彙        = bm25 / (bm25 + RAG_BM25_SATURATION)            RAG_BM25_SATURATION = 4.0
              # 候補プールに依存しない飽和（min-max ではない）：絶対的な関連度を保ち、
              # 弱いプールでも「証拠なし / 最低統合スコア」しきい値が意味を持つ

symbol_boost = 0.5 × (0.85 + 0.15 × rarity)   クエリ語がチャンクのシンボルと完全一致
             = 0.5 × 0.35 × rarity            クエリ語がシンボルの一部と部分一致
             rarity = min(1, idf / 6)         # 完全な識別子クエリは高精度な意図
...
q_weight      = idf[token] × dynamic_noise_penalty[token]   # idf はクエリ時に一度だけ適用
postings      = チャンクごとの生の出現頻度（長さ正規化は BM25 の分母）

dynamic_noise_penalty ∈ [0.10, 1.0]  — 静的ストップワードリストではなくコーパス単位で計算
```

> インデックススナップショットはバージョン付き（`bm25-v1`）。形式不一致時には永続化された `documents.json` / `chunks.json` という真の出所から透過的に再構築される——データ損失も手動移行もなし。
...
flowchart TD
  ...
  subgraph F["FAST — 精確検索"]
    F1["BM25（生の出現頻度の転置リスト）"] --> F2["スコア: 語彙 × 0.80 + graph_bonus + symbol_boost"]
    F2 --> F3["上位 K チャンクを返す"]
  end
...
| IDF スムージング | `log(N/df)` | 飽和 BM25 | N/A | **BM25 `log(1+(N−df+0.5)/(df+0.5))`** |
| TF 飽和 | なし | BM25 k₁ パラメータ | N/A | **BM25 k₁/b 長さ正規化 + 飽和 `raw/(raw+4)`** |
| シンボル完全一致の意図 | ✗ | ✗ | 近似 | **完全識別子ブースト（idf 希少度でスケール）** |
...
| 言語横断の召回 | ✗ | ✗ | 部分的（多言語モデル） | **類義語グループ + クエリ時翻訳ブリッジ** |
| コードネイティブグラフ | ✗ | ✗ | ✗ | **インポートエッジ + シンボルテーブル + 行範囲（ネストシンボル）** |
| クエリルーティング | 固定 | 固定 | 固定 | **自動: fast / global / hybrid** |
| 未知語処理 | 失敗 | 失敗 | 埋め込みで対応 | エンティティ抽出で対応 |
| 説明可能性 | スコア分解 | スコア分解 | ブラックボックス | **完全スコア内訳: 語彙 + エンティティ + グラフ + コミュニティ + シンボル** |
...
### 4.2 RAG 強化（語彙のみ、埋め込みなし）

集中的な強化が、検索を完全に語彙のまま保ちつつ両 RAG サブシステムをアップグレードしました：

- **BM25 ランキング + シンボル完全一致ブースト** — 語彙コアを TF-IDF コサインから BM25（`k1=1.5`、`b=0.6`）へ、プール非依存の飽和とともに移行し、完全/部分の識別子ブーストを追加。関数やクラスを名指しするクエリが定義チャンクを高精度に表面化する。
- **構造を保つコードチャンク化** — Python チャンカーがネストした関数・クロージャ・デコレート補助関数・内部クラスへ再帰（深さ ≤ 4）し、`outer.inner`・`Class.method` の階層名を付け、各チャンクの `symbol`/`symbol_kind` を保持してシンボルブーストから参照可能にする。約 40 言語を索引（Python は AST 認識、他は汎用コードチャンカー）。
- **境界安全なドキュメントチャンク化** — 過大ブロックを最寄りの段落 / 文 / 改行 / 空白境界へ戻し、境界整合のオーバーラップを付ける。文字途中での切断を廃止。
- **言語横断のナレッジ検索** — 中国語で検索して英語コンテンツに当てる（逆も）。通常の BM25 上で動作し、埋め込みモデル不要。約 4000 グループの中/英/略語類義語表をインデックス時・クエリ時に拡張し、加えてクエリ時翻訳ブリッジ `_augment_query_cross_lingual` が単一文字種クエリを検出してモデルのベストエフォート翻訳を追記、一本のバイリンガルクエリがトークナイザを一度で通る。対象はナレッジ RAG のみ、コード検索はリテラルのまま。
- **`rag_remember` エージェント自己取り込みツール** — エージェントが良い参考や蒸留した知見をナレッジライブラリへ直接書ける（`rag_remember(text, title?, tags?, category?)`）。管理画面のテキスト取り込みと同じ重複排除/取り込み経路（追加の LLM 呼び出しなし・sha256 重複排除）を再利用。ナレッジライブラリのみに書き込むため plan / 調査モードでも許可。
- **書籍級の取り込み** — `RAG_MAX_CHUNKS_PER_DOC` を 20000 へ引き上げ、統一の `RAG_MAX_DOCUMENT_CHARS`（既定 12M、環境変数で上書き可）が各パーサに散在していた切り捨てを置き換え、本一冊を安全に読み込む。解析はハードタイムアウト付きサブプロセスで実行され、チャンク上限が極端入力を抑える。
- **バージョン付きインデックススナップショット** — スナップショットは `bm25-v1` 形式タグを持ち、不一致時は永続化されたドキュメント/チャンクの真の出所から透過的に再構築。
