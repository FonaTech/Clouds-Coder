...
DEFAULT_LAYOUT: dict[str, list[str]] = {
    ...
    "ide/auth.py": ["IDEAuthError", "IDEAuthStore"],
    "ide/errors.py": ["IDECapabilityError", "IDEFileConflict"],
    "ide/events.py": ["ide_public_operation_data"],
    "ide/preview.py": [
        "normalize_rel_preview_path", "is_code_preview_candidate", "preview_kind_for_path",
        "normalize_markdown_preview_text", "build_code_preview_rows",
        "workspace_file_revision_map", "workspace_revision_delta",
        "~^_preview_markdown_",
    ],
    "ide/sandbox.py": [
        "WINDOWS_JOB_SANDBOX_MARKER",
        "~^_IDE_SANDBOX_", "~^_WINDOWS_LOW_INTEGRITY_",
        "~^_(?:is_windows_job_sandbox_prefix|windows_|popen_windows_sandboxed|run_windows_sandboxed_command|detect_ide_sandbox_backend)$",
    ],
    ...
    "agent/tools.py": [
        ...
        "_ask_user_option_rows", "_ask_user_option_value",
    ...
    "mcp/driver.py": ["~^_?mcp_", "MCPServerProcess", "MCPManager"],
...
class AutoLayoutGenerator:
    ...
    CLASS_RULES: list[tuple[re.Pattern[str], str]] = [
        (re.compile(r"^IDEAuth"), "ide/auth.py"),
        (re.compile(r"^IDE.*(?:Error|Conflict)$"), "ide/errors.py"),
        (re.compile(r"^IDE"), "ide/handler.py"),
    ...
    def _classify(self, node: NodeInfo) -> str:
        ...
        if name == "ide_public_operation_data":
            return "ide/events.py"
        if name.startswith("_ask_user_"):
            return "agent/tools.py"
        if (
            name.startswith("_preview_markdown_")
            or name.startswith("workspace_")
            or name in {
                "normalize_rel_preview_path", "is_code_preview_candidate",
                "preview_kind_for_path", "normalize_markdown_preview_text",
                "build_code_preview_rows",
            }
        ):
            return "ide/preview.py"
        if (
            name.startswith(("_IDE_SANDBOX_", "_WINDOWS_LOW_INTEGRITY_", "_windows_"))
            or name in {
                "WINDOWS_JOB_SANDBOX_MARKER", "_is_windows_job_sandbox_prefix",
                "_popen_windows_sandboxed", "_run_windows_sandboxed_command",
                "_detect_ide_sandbox_backend",
            }
        ):
            return "ide/sandbox.py"
        if node.kind == "class":
        ...
        if name.startswith(("mcp_", "_mcp_", "MCP")):
