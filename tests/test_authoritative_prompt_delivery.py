...
class AuthoritativePromptDeliveryTests(unittest.TestCase):
    ...
    def test_all_execution_topologies_receive_complete_goal_at_model_boundary(self):
        ...
        bind(
            session,
            "_estimate_model_call_prompt_tokens",
    ...
    def test_plan_choice_preserves_original_authoritative_goal(self):
        ...
        self.assertTrue(
            session.runtime_authoritative_goal.endswith("TAIL_ACCEPTANCE_MARKER_9F72")
        )
    ...
    def test_multi_agent_seed_keeps_full_goal_for_explorer_and_manager(self):
        ...
        bind(
            session, "_mark_multi_agent_context_hud_active", lambda self, **kwargs: None
        )
        bind(session, "_agent_language_policy_note", lambda self: "")
        bind(session, "_ensure_blackboard", lambda self: self.blackboard)
        bind(
            session,
            "_blackboard_reset_for_goal",
            lambda self, goal, **kwargs: self.blackboard.update(original_goal=goal),
        )
        ...
        bind(
            ...
            "_agent_context",
            lambda self, role: [
                row for row in appended if row.get("agent_role") == role
            ],
        )
        ...
        manager_text = "\n".join(
            session._model_message_text(row) for row in session.manager_context
        )
    ...
    def test_authoritative_marker_survives_tier3_compaction_and_offload(self):
        ...
        bind(
            ...
            lambda self, *args, **kwargs: (_ for _ in ()).throw(
                AssertionError("must not offload authority")
            ),
        )
        ...
        self.assertIn(
            "TAIL_ACCEPTANCE_MARKER_9F72", session.manager_context[0]["content"]
        )
