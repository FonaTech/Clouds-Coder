...
class SingleNoPlanTodoBootstrapTests(unittest.TestCase):
    ...
    def test_adaptive_bootstrap_exposes_read_only_tools_without_mutations(self):
        session = self.bare_session()
        bind(session, "_available_tools", lambda self: list(cc.TOOLS))
        names = [
            row["function"]["name"]
            for row in session._single_no_plan_todo_bootstrap_tools(include_perception=True)
        ]
        self.assertIn("TodoWrite", names)
        self.assertIn("TodoWriteRescue", names)
        self.assertIn("read_file", names)
