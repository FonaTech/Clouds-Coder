...
def _write_skill(root: Path, folder: str, frontmatter: str, body: str = "workflow") -> None:
...
class SkillSelectionTests(unittest.TestCase):
    ...
    def test_recall_searches_all_metadata_not_first_thirty(self):
        for index in range(35):
            _write_skill(
                self.root,
                f"generic-{index:02d}",
                f"name: generic-{index:02d}\ndescription: Generic unrelated workflow {index}",
            )
        _write_skill(
            self.root,
            "z-special",
            "name: z-special\ndescription: Specialized quasar telemetry\ntriggers: [quasar telemetry]",
        )
    ...
    def test_canonicalization_accepts_id_name_alias_and_case(self):
        ...
        for requested in ("local:Frontend-Pro", "Frontend-Pro", "frontend-pro", "UI Builder", "ui builder"):
    ...
    def test_unknown_and_ambiguous_names_are_structured_errors(self):
        _write_skill(self.root, "one", "name: one\ndescription: First\naliases: [shared]")
        _write_skill(self.root, "two", "name: two\ndescription: Second\naliases: [shared]")
        store = cc.SkillStore(self.root)
    ...
    def test_negative_trigger_filters_candidate(self):
        ...
        self.assertEqual(store.recall_metadata("make a browser report")[0]["id"], "local:browser")

    def test_recall_deduplicates_tokens_and_does_not_substring_match_ide(self):
        _write_skill(
            self.root,
            "video-comprehension",
            "name: video-comprehension\ndescription: Analyze video and screen recordings",
        )
        _write_skill(
            self.root,
            "telemetry",
        ...
        repeated = store.recall_metadata("quasar quasar telemetry telemetry")[0]["score"]
    ...
    def _bare_session(self, store: cc.SkillStore):
        ...
        session._ensure_blackboard = types.MethodType(lambda self: self.blackboard, session)
        session._ensure_skills_ready = types.MethodType(lambda self, force=False: None, session)
        session._emit = types.MethodType(lambda self, kind, payload: self.events.append((kind, payload)), session)
        session._active_skill_step_id = types.MethodType(lambda self, board=None: "step-1", session)
        session._tier_agent_context_limits = types.MethodType(lambda self, _tier: {"agent_messages": 100}, session)
        session._context_budget_tier_for_dynamic_memory = types.MethodType(lambda self: 1, session)
        return session
    ...
    def _runtime_session(self, store: cc.SkillStore, *, mode: str, plan: bool):
        ...
        def chat(messages, **kwargs):
            ...
            focused = payload["step_text"] or " ".join(row["content"] for row in payload["active_todos"])
            recalled = store.recall_metadata(focused, limit=1)
            selected = recalled[0]["id"] if recalled else ""
            active = {row["id"] for row in payload["loaded_skills"]}
            recommendation = {
                "skill_id": selected, "purpose": "current workflow matches metadata",
                "confidence": 0.95, "evidence": [focused],
            }
            result = {
                "step_id": payload["step_id"], "assessment": "specialized",
                "load": [recommendation] if selected and selected not in active else [],
                "keep": [{"skill_id": selected, "purpose": "still needed"}] if selected in active else [],
                "unload": [{"skill_id": key, "purpose": "current workflow no longer needs it", "confidence": 0.95, "evidence": [focused]} for key in active if key != selected],
                "discover": [], "uncertainties": [],
            }
            return {"content": json.dumps(result)}
        session.ollama = types.SimpleNamespace(chat=chat)
        session._latest_user_goal_text = types.MethodType(
            lambda self: "Build a scientific simulation application",
            session,
        )
        session._active_skill_step_id = types.MethodType(cc.SessionState._active_skill_step_id, session)
        if plan:
    ...
    def test_manual_load_pins_duplicate_does_not_reinject_and_unload_is_blocked(self):
        ...
        second = session._load_skill_with_cache("local:api", load_source="manual:developer")
        ...
        self.assertIn("cannot be unloaded", session._unload_skill("api"))
        self.assertIn("local:api", session.blackboard["loaded_skills"])
        ...
        for plan in (False, True):
            for mode in (cc.EXECUTION_MODE_SINGLE, cc.EXECUTION_MODE_SYNC):
                with self.subTest(plan=plan, mode=mode):
                    ...
                    selection = session._refresh_loaded_skills_for_execution_focus(trigger="run-start")

                    self.assertIn("local:fluid-solver", session.blackboard["loaded_skills"])
                    ...
                    self.assertEqual(selection["selection_order"], ["local:fluid-solver"])
                    diagnostics = [payload for kind, payload in session.events if kind == "skill_selection"][-1]
                    ...
                    manager_context = session._loaded_skills_context_block(for_role="manager")
                    worker_context = session._loaded_skills_context_block(for_role="developer")
                    self.assertIn("Use pressure projection", manager_context)
                    self.assertIn("Use pressure projection", worker_context)

                    event_count = len([event for event in session.events if event[0] == "skill_selection"])
                    repeated = session._refresh_loaded_skills_for_execution_focus(trigger="manager-round")
                    self.assertTrue(repeated["skipped"])
                    self.assertEqual(
                        len([event for event in session.events if event[0] == "skill_selection"]),
                        event_count,
                    )
    ...
    def test_no_plan_todo_transition_replaces_active_skill(self):
        ...
        _write_skill(
            self.root,
            "slides",
            "name: slides\ndescription: Presentation workflow\ntriggers: [PowerPoint deck]",
        )
        store = cc.SkillStore(self.root)
        session = self._runtime_session(store, mode=cc.EXECUTION_MODE_SINGLE, plan=False)

        session._refresh_loaded_skills_for_execution_focus(trigger="first-todo")
        self.assertEqual(set(session.blackboard["loaded_skills"]), {"local:fluid-solver"})
        session.todo.update([
            {"content": "实现 Navier-Stokes 压力投影流体求解器", "status": "completed"},
            {"content": "Create a PowerPoint deck", "status": "in_progress"},
        ])
        session._refresh_loaded_skills_for_execution_focus(trigger="todo-focus-transition")
    ...
    def test_todo_dispatch_refreshes_when_no_plan_focus_changes(self):
        _write_skill(self.root, "fluid", "name: fluid\ndescription: Fluid\ntriggers: [Navier-Stokes]")
        store = cc.SkillStore(self.root)
        session = self._runtime_session(store, mode=cc.EXECUTION_MODE_SINGLE, plan=False)
        session.todo.items = []
        refreshes = []
        session._capture_todo_write_transaction = types.MethodType(lambda self, *args, **kwargs: {}, session)
        session._get_active_plan_step = types.MethodType(lambda self, board=None: None, session)
        session._todo_route_kind = types.MethodType(lambda self, **kwargs: "pure_single", session)
        ...
        session._dispatch_todo_update(
            {"items": [{"content": "实现 Navier-Stokes 求解器", "status": "in_progress"}]},
            role="developer",
        )
